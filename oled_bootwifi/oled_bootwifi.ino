...
// BOOT button on many ESP32-C3 boards is GPIO9 and active-low.
#define BOOT_BUTTON_PIN 9
#define BOOT_BUTTON_ACTIVE_LOW 1
#define BOOT_DEBOUNCE_MS 45
...
unsigned long wifiStatusUntilMs = 0;
...
void handleBootButton() {
  ...
  bool pressed = BOOT_BUTTON_ACTIVE_LOW ? (digitalRead(BOOT_BUTTON_PIN) == LOW) : (digitalRead(BOOT_BUTTON_PIN) == HIGH);
  ...
  if (millis() - changedAt < BOOT_DEBOUNCE_MS) return;

  if (pressed != stablePressed) {
    stablePressed = pressed;
    if (stablePressed) toggleWifiAp();
  }
}
...
void drawOled() {
  ...
  if (wifiStatusUntilMs != 0 && millis() < wifiStatusUntilMs) {
    oled.text(0, 0, "WIFI");
    oled.hline(0, 10, 72);
    oled.text(12, 18, wifiApOn ? "ON" : "OFF", 2);
    oled.send();
    return;
  }
...
void loop() {
  handleBootButton();
  if (wifiApOn) server.handleClient();
  updateAlarm();

  unsigned long now = millis();
  if (wifiStatusUntilMs != 0 && now >= wifiStatusUntilMs) {
    wifiStatusUntilMs = 0;
    drawOled();
  }
